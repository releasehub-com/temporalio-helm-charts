...
#
# Before upgrading, we need to update the schema on the temporal and visibility databases
# This is done on the temporal-admintools pod, using commands like the following:
#
#  temporal-sql-tool \
#    --user temporal \
#    --port 5432 \
#    --plugin postgres \
#    --password wPrHkZiOvyK5T8CLmjDJ \
#    --endpoint temporal-${db_endpoint}.clzc7zni9qdc.us-west-2.rds.amazonaws.com \
#    update \
#    --schema-dir ./schema/postgresql/v96/temporal/versioned
#
#  temporal-sql-tool \
#    --user temporal \
#    --port 5432 \
...
#    --endpoint temporal-${db_endpoint}.clzc7zni9qdc.us-west-2.rds.amazonaws.com \
#    update \
#    --schema-dir ./schema/postgresql/v96/visibility/versioned

db_endpoint=$1

: ${db_endpoint:?Missing db endpoint}

read -p "Have you run the schema upgrades for the target version? [yn] " -n 1 -r
if [[ $REPLY =~ ^[Yy]$ ]]; then
	helm \
		upgrade \
		temporal \
		...
		--set server.history.affinity.podAntiAffinity.preferredDuringSchedulingIgnoredDuringExecution[0].podAffinityTerm.labelSelector.matchExpressions[0].operator=temporal \
